...
public class SistemaArma : MonoBehaviour
{
    ...
    // Referência ao Animator da arma (arraste o GameObject da arma aqui no Inspector)
    [Header("Animação")]
    [SerializeField] Animator animArma;
 
    // Nome do trigger definido no Animator Controller da arma
    private static readonly int TriggerAtirar = Animator.StringToHash("atirar");
    ...
    void Update()
    {
        ...
        if (Input.GetMouseButton(0) && podeAtirar)
        {
            ...
            if (animArma != null)
                animArma.SetTrigger(TriggerAtirar);
